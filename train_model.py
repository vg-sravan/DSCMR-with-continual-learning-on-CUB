...
def train_model(model, data_loaders, optimizer, alpha, beta, namee, model_weights=None, device="cpu", num_epochs=10):
    ...
    print(f'Training for dataset {namee} ..............')
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'test']:
            ...
            # Iterate over data.
            print(phase)
            for imgs, txts, labels, _ in data_loaders[phase]:
                # imgs = imgs.to(device)
                # txts = txts.to(device)
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # Forward
                    view1_feature, view2_feature, view1_predict, view2_predict = model(
                        imgs, txts)
#                     print('forward complete\n')
                ...
                running_corrects_img += torch.sum(torch.argmax(
                    img_preds, dim=1) == torch.argmax(labels, dim=1))
                running_corrects_txt += torch.sum(torch.argmax(
                    txt_preds, dim=1) == torch.argmax(labels, dim=1))

            epoch_loss = running_loss / len(data_loaders[phase].dataset)
            epoch_img_acc = running_corrects_img.double(
            ) / len(data_loaders[phase].dataset)
            epoch_txt_acc = running_corrects_txt.double(
            ) / len(data_loaders[phase].dataset)

            print(
                f'Loss = {epoch_loss} Accuracy for imgs = {epoch_img_acc} and Accuracy for txts = {epoch_txt_acc} ')
            if(phase == 'test'):
                for testing_phase in ['train', 'test']:
                    t_imgs, t_txts, t_labels = [], [], []
                    with torch.no_grad():
                        for imgs, txts, labels, _ in data_loaders[testing_phase]:
                            if torch.cuda.is_available():
                                #                         imgs = imgs.cuda()
                                #                         txts = txts.cuda()
                                #                         labels = labels.cuda()
                    ...
                    print('{} Img2Txt: {:.4f}  Txt2Img: {:.4f}'.format(
                        testing_phase, img2text, txt2img))
    ...
    # you can save the weights if u want
    return model, test_img_acc_history, test_txt_acc_history, epoch_loss_history


def testing_model(model, dataloader, namee):

    # '''Testing the model on the test data of the given dataloader'''
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    print(f'Testing on test dataset of dataset {namee}......................')
    t_imgs, t_txts, t_labels = [], [], []
    with torch.no_grad():
        for imgs, txts, labels, _ in dataloader['test']:
            if torch.cuda.is_available():
                #                         imgs = imgs.cuda()
                #                         txts = txts.cuda()
                #                         labels = labels.cuda()
    ...
    img2text = fx_calc_map_label(t_imgs, t_txts, t_labels)
    txt2img = fx_calc_map_label(t_txts, t_imgs, t_labels)
    avg_mAP = (img2text+txt2img)/2

    print('mAP on test set of dataset {} -->  Img2Txt: {:.4f}  Txt2Img: {:.4f} Avg_mAP: {:.4f}'.format(
        namee, img2text, txt2img, avg_mAP))
    print('Testing Done...............')
    return txt2img
